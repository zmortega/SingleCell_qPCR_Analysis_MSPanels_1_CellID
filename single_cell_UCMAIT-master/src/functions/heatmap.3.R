#### heatmap.3 function from Obi Griffith ####
...
nrow(x)")
      lmat <- cbind(lmat[, 1] + 1, c(rep(NA, nrow(lmat) - 1), 
                                     1), lmat[, 2] + 1)
      lwid <- c(lwid[1], 0.075*ncol(RowSideColors), lwid[2])
    }
...
#     plot.index <- plot.index + 1
    if (!is.matrix(RowSideColors)){
      ...
      par(mar = c(margins[1], 0, 0, 0.5))
      # keep it as a matrix even if there is only 1 column
      ...
      if (length(colnames(RowSideColors)) > 0) {
        n_side <- dim(rsc)[2]
        at <- if (n_side == 1) 0.5 else 0:(n_side - 1)/(n_side - 1)
        axis(1, at,
...
#     plot.index <- plot.index + 1
  ...
  if(!is.null(xlab)) mtext(xlab, side = 1, line = margins[1] - 1.15, cex=1.5)
  if(!is.null(ylab)) mtext(ylab, side = 4, line = margins[2] - 1.15, cex=1.42)
  ...
  if(key)
  {
    ...
    if (!is.na(key.xlab)) {
      mtext(side=1, key.xlab, line=par("mgp")[1], padj=0.5, cex=par("cex") * par("cex.lab") + 0.5)
    }
    ...
    if(density.info=="density")
    {
      ...
      if (is.null(key.ylab))
        key.ylab <- "Density"
      if (!is.na(key.ylab))
        mtext(side=2,key.ylab, line=par("mgp")[1] + 0.5, padj=0.5, cex=par("cex") * par("cex.lab") + 0.65)
    }
