...
clusterFilter_2nd <- function(ctInput, testK = F, numCenters = 2, plotHeatmap=F, plotClustOnly=F, 
  ...
  if (Species == "HU") {
    ...
    Panel3<- c()
    Panel1and3<- sort(c(Panel1, CellID))
  }
  ...
  print(paste0("The panel determined in the 'Panel Detection' tests, in the 'clusterFilter.R' script, is ", PanelNumber))
  
  if (PanelNumberLoaded == PanelNumber) {
    print("Good news: The panel detected in ctInput is the same as the panel loaded by the User.")
  } else {
    warning(print("Warning! The panel detected and the panel number input by the user are not the same!"))
  }

###SMS edit, now that we know what panel is loaded into "ctInput," we can then jump into the HLADR test below but only if the "Species" setting is "HU". For "MS", we have our own 
  # if (Species == "HU") {
  #   if (PanelNumber == "1" | PanelNumber == "1 and 3") {
  ...
  ###DASH TEST: you'll have to change the HLADR test to a DASH test which applies to any panel from any species. R is converting "-" to "." and that's screwing up the Tests below.
  DashtestPanel1<- any(grepl("-", Panel1) == TRUE)
  DashtestPanel1and3 <- any(grepl("-", Panel1and3) == TRUE)
  DashtestPanelCellID<- any(grepl("-", CellID) == TRUE)
  
  if (PanelNumber == "1" && DashtestPanel1 == TRUE) {
    ###Assemble a vector with the genes that have dashes in their name
    GenesDashed<- Panel1[grep("-", Panel1)]
    print(paste(c("Which genes are dashed in the panel? ",GenesDashed),sep = "",collapse = " "))
    ###Change the dashes to dots
    GenesDotted<- str_replace(GenesDashed, "-", ".")
    ###Search the column names for ctInput for the genes in GenesDotted
    GenestoChangeNumbers<- grep(paste(GenesDotted, collapse = "|"), colnames(ctInput))
    ###Find the position that the genes are in the vector (colnames(ctInput))
    for (i in 1:length(GenesDotted)) {
      names(ctInput)[which(GenesDotted[i]==colnames(ctInput))] <- str_replace(colnames(ctInput)[which(GenesDotted[i]==colnames(ctInput))], "\\.", "-")
    }
  } else if (PanelNumber == "1 and 3" && DashtestPanel1and3 == TRUE) {
    ###Assemble a vector with the genes that have dashes in their name
    GenesDashed<- Panel1and3[grep("-", Panel1and3)]
    print(paste(c("Which genes are dashed in the panel? ",GenesDashed),sep = "",collapse = " "))
    ###Change the dashes to dots
    GenesDotted<- str_replace(GenesDashed, "-", ".")
    ###Search the column names for ctInput for the genes in GenesDotted
  ...
  } else if (PanelNumber == "CellID" && DashtestPanelCellID == TRUE) {
  
    ###Assemble a vector with the genes that have dashes in their name
    GenesDashed<- CellID[grep("-", CellID)]
    print(paste(c("Which genes are dashed in the panel? ",GenesDashed),sep = "",collapse = " "))
    ###Change the dashes to dots
    GenesDotted<- str_replace(GenesDashed, "-", ".")
    ###Search the column names for ctInput for the genes in GenesDotted
    GenestoChangeNumbers<- grep(paste(GenesDotted, collapse = "|"), colnames(ctInput))
    ###Find the position of the dotted genes  in the vector (colnames(ctInput)) and replace with a dash
    for (i in 1:length(GenesDotted)) {
      names(ctInput)[which(GenesDotted[i]==colnames(ctInput))] <- str_replace(colnames(ctInput)[which(GenesDotted[i]==colnames(ctInput))], "\\.", "-")
    }
    print(colnames(ctInput))
  ...
  # DELETE: No need for a K-means test for the first clustering flunction
  ...
  FirstGene<- (grep("kmeans.cluster", colnames(ctInput))+1)
  LastGene<- ncol(ctInput)

  Test1<- setequal(Panel1, colnames(ctInput)[FirstGene:LastGene])
  print(paste0("Test 1 is ", Test1))
  Test2<- setequal(Panel3, colnames(ctInput)[FirstGene:LastGene])
  print(paste0("Test 2 is ", Test2))
  Test1and2<- setequal(Panel1and3, colnames(ctInput)[FirstGene:LastGene])
  ...
  ###Here, we're going to use the pattern that the metadata is in the first 9 columns and the genes follow, to subset. Because this data has already gone through one clustering function
  ###it no longer makes sense to rely on the Panel number. Simply find the kmean.cluster column, as the next column is the first gene  and then find the end of the dataframe.
  ###That will be where the genes in the dataframe are.
  ###Because this data has already gone through one clustering function
  ###it no longer makes sense to rely on the Panel number. Simply find
  ###the kmeans.cluster column (the next column is the first gene) and
  ###then go to the end of the dataframe.
  TopNumber   <- grep("kmeans.cluster", colnames(ctInput)) + 1
  BottomNumber <- ncol(ctInput)
  
  print(paste0("The first column you'll pull is: ",
               colnames(ctInput)[TopNumber]))
  print(paste0("The last column you'll pull is: ",
               colnames(ctInput)[BottomNumber]))
  print(colnames(ctInput)[TopNumber:BottomNumber])
  ...
  ###SMS edit to use the panel number detected above to subset columns based on patterns, not column numbers. These if statements no longer have purpose, but I kept them to avoid
  ###screwing up the code too much with too many deletions.
  if (Species == "HU") {
      if (PanelNumber == "1") {
        FirstColumn<- TopNumber
        LastColumn<- BottomNumber
        ctTestK <- ctInput[,c(FirstColumn:LastColumn)]
        print(paste("PanelNumber equals:", PanelNumber, ". Columns to be sent for kmeans testing: ", colnames(ctInput)[TopNumber], " and ", colnames(ctInput[BottomNumber])))
  ...
  ctClust <- data.frame(ctInput, normFit$cluster, check.names = FALSE)
  
  if (Troubleshoot == TRUE) {
    print("Column Names for ctClust after adding the 'normFit$cluster' to the dataframe are: ")
    print(colnames(ctClust))
  }
  
  ###Drop the first kmeans.cluster column.
  DropColumn<- grep("kmeans.cluster", colnames(ctClust))
  ctClust<- ctClust[,c(-DropColumn)]
  ...
  if(plotHeatmap == T){
    ...
    ###Trying to find the first gene and last gene again. 
    ...
    ctClust <- ctClust[,c(1:(idCols), (order(pvals)+idCols))] #, ncol(ctClust))] ##SMS edit, removing the ncol function because we don't need it anymore
    ...
    uniquePatients <- unique(ctClust[,"patient"])
...
#               ylab=heatmapYLabel)
    if(plotClustOnly == FALSE){
      ...
      #print(as.matrix(ctClust[,(FirstGene):(LastGene)]))
      ...
      if(plotClustOnly == TRUE){
        heatmap.2(as.matrix(ctClust[(FirstGene:LastGene)]), Colv=F, ##LG settings for as.matrix(ctClust[(idCols + 1):(ncol(ctClust)-1)])
                  dendrogram="row", trace="none", 
                  col=colorpanel(15, "blue", "white", "red"),
